...
fn main() {
    ...
    let mut machine2 = StateMachine::<B256, B256, 32, 32>::new(DefaultConfig::default_config());
    ...
    let mut trace_record = vec![];
    // Execute the program
    for instruction in program2 {
        machine2.exec(&instruction);
    }
    // Print the trace record (prettified), sorted by ascending time by default
    for x in machine2.trace().into_iter() {
        trace_record.push(x);
    }
    println!("{}", trace_record.len());
    println!("Verifying memory consistency...");
    // If build_and_test_circuit does not panic, then the trace is valid.
    build_and_test_circuit_with_time(trace_record, 11);
    println!("Memory consistency check done. The execution trace is valid.");
